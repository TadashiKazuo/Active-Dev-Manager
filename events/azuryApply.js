...
let tckCategory = '962270401348649000'
    let tckStaff    = '962255185026105364'
    let tckAdmin    = '962255185026105364'
    if(interaction.customId == `azu_apply`) {
      ...
      if(interaction.member.roles.cache.some(x => x.id == "962255352219439144")) return interaction.reply({ content: `❌ **You already are a staff member!**`, ephemeral: true })
    const tckEmb = new MessageEmbed()
      ...
      if(!interaction.member.roles.cache.some(x => x.id == tckStaff)) return interaction.reply({ content: `🔐・You must have the **<@&${tckStaff}>** role to be able to use this option!`, ephemeral: true })
          ...
          user.roles.add("962255352219439144")
          user.roles.add("962255352219439144")
      ...
      setTimeout(function(){
          ...
          client.channels.cache.get(`962259257573650432`).send({ content: `<a:white_Exclamation:931664180254232626> **New Staff Member: ${user}** <a:white_Exclamation:931664180254232626>\n> **A new member of the server has now become a staff member! Congrat them in chat and show them around!**`, components: [ad_btn] })
